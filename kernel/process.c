    ...
    switch (parent->mapped_info[i].seg_type) {
      case DATA_SEGMENT:
        ...
        for (int i = 0; i < parent->mapped_info[i].npages; i++) {
          ...
          // sprint("copying data segment from parent to child, va: 0x%lx, pa: "
          //        "0x%lx\n",
          //        parent->mapped_info[i].va + i * PGSIZE, child_pa);
          uint64 pa = lookup_pa(parent->pagetable,
                                parent->mapped_info[i].va + i * PGSIZE);
          memcpy(child_pa, (void*)pa, PGSIZE);
          user_vm_map(child->pagetable, parent->mapped_info[i].va + i * PGSIZE,
      ...
      case CODE_SEGMENT: {
        ...
        // mapped_info page of parent's process structure. use the information
        // in mapped_info to retrieve the virtual to physical mapping of code
        ...
        user_vm_map((pagetable_t)child->pagetable, parent->mapped_info[i].va,
                    parent->mapped_info[i].npages * PGSIZE, pa,
...
int do_wait(uint64 pid) {
  ...
  if (pid == -1) {
    bool has_child = 0;
    for (int i = 0; i < NPROC; i++) {
      if (current != procs[i].parent) continue;
      has_child = 1;
