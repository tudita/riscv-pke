...
void *user_va_to_pa(pagetable_t page_dir, void *va) {
  ...
  // should return NULL.
  int64 p = lookup_pa(page_dir, (uint64)va);
  if (p == 0) return NULL;
  uint64 pa = p + ((uint64)va & ((1 << PGSHIFT) - 1));
  return (void *)pa;
...
void user_vm_unmap(pagetable_t page_dir, uint64 va, uint64 size, int free) {
  ...
  // in lab2_2.\n" );
  for (uint64 i = va; i < va + size; i += PGSIZE) {
    pte_t *pte = page_walk(page_dir, i, 0);
    if (pte == 0) continue;             // 未找到
    if ((*pte & PTE_V) == 0) continue;  // 已经invalid
    if (free) {
